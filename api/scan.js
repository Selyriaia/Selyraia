...
// -----------------------------------------------------------------------------
// POURQUOI CE DÉCOUPAGE : une fonction Netlify est coupée au bout de ~10 secondes.
...
async function callMistral(p, k, m) {
  const r = await fetch('https://api.mistral.ai/v1/chat/completions', {
    ...
    body: JSON.stringify({ model: m, messages: [{ role: 'user', content: p }], max_tokens: 900 })
  });
...
async function callClaude(p, k, m) {
  const r = await fetch('https://api.anthropic.com/v1/messages', {
    ...
    body: JSON.stringify({ model: m, max_tokens: 900, messages: [{ role: 'user', content: p }] })
  });
...
async function callGemini(p, k, m) {
  const r = await fetch(`https://generativelanguage.googleapis.com/v1beta/models/${m}:generateContent?key=${k}`, {
    ...
    body: JSON.stringify({ contents: [{ parts: [{ text: p }] }], generationConfig: { maxOutputTokens: 900 } })
  });
...
export default async (request) => {
  ...
  if (svcKey) {
    ...
    return fail("SUPABASE_SERVICE_KEY absente des variables Netlify.", 500);
  }
  ...
  if (action === 'me') {
    return ok({ email: user.email, plan, isAdmin: admin, scansUsed: used, quota,
                canScan: admin || used < quota, dbError: admin ? dbError : null });
  }

  const engine = ENGINES[b.engine] ? b.engine : 'mistral';
...
Règles : "competitors" = noms réellement cités à la place de ${brand}, max 5, vide si aucun. "problems" = 2 à 4. "errors" = uniquement les inexactitudes réellement visibles, vide si aucune. "actions" = exactement 2, les plus prioritaires.`;
    ...
    try {
      let raw = (await ask(engine, p)).replace(/```json|```/g, '').trim();
